...
class ModifierMediaRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            $fichierExistant = $media?->fichier;
            $urlExistante = $media?->url_externe;

            if (!$fichier && !$urlExterne && !$fichierExistant && !$urlExistante) {
                $validator->errors()->add('fichier', 'Vous devez conserver ou fournir un fichier ou un lien externe.');
            }

            if ($typeMedia === 'lien' && empty($urlExterne) && empty($urlExistante)) {
