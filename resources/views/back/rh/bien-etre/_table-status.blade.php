        ...
        <div class="content-card">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
                ...
                <a href="{{ route('rh.bien-etre.index') }}" class="btn btn-outline-secondary rounded-pill px-4">
                    <i class="fa-solid fa-arrow-left me-2"></i>Tous les dossiers
                </a>
            </div>
            ...
            <div class="table-responsive">
                <table class="table align-middle custom-table">
                    ...
                    <tbody>
                        @forelse($dossiersList as $dossier)
                            <tr>
                                <td>{{ $dossier->id }}</td>
                                <td>{{ $dossier->titre }}</td>
                                <td>{{ optional($dossier->membreEquipe)->nom }} {{ optional($dossier->membreEquipe)->prenom }}</td>
                                <td>{{ ucfirst(str_replace('_', ' ', $dossier->type)) }}</td>
                                <td>{{ ucfirst($dossier->niveau_priorite) }}</td>
                                <td>
                            ...
                            <tr>
                                <td colspan="7" class="text-center py-5">
                                    <div class="empty-state">
                                        ...
                                        <h5 class="mt-3">Aucun dossier</h5>
