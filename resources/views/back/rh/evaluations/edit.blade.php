...
@section('page_title', 'Modifier l’évaluation')
@section('page_subtitle', 'Mets à jour l’évaluation, sa note, son statut et son contenu qualitatif.')
    ...
    <div class="row g-4">
        <div class="col-12">
            <div class="content-card">
                ...
                <form method="POST" action="{{ route('rh.evaluations.update', $evaluation) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <select name="membre_equipe_id" class="form-select custom-input @error('membre_equipe_id') is-invalid @enderror">
                                ...
                                @foreach($membres as $membre)
                                    <option value="{{ $membre->id }}" @selected(old('membre_equipe_id', $evaluation->membre_equipe_id) == $membre->id)>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="evaluateur_id" class="form-select custom-input @error('evaluateur_id') is-invalid @enderror">
                                ...
                                @foreach($evaluateurs as $evaluateur)
                                    <option value="{{ $evaluateur->id }}" @selected(old('evaluateur_id', $evaluation->evaluateur_id) == $evaluateur->id)>
                                        {{ $evaluateur->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="date" name="date_evaluation" class="form-control custom-input @error('date_evaluation') is-invalid @enderror"
                                   value="{{ old('date_evaluation', optional($evaluation->date_evaluation)->format('Y-m-d')) }}">
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="statut" class="form-select custom-input @error('statut') is-invalid @enderror">
                                @foreach($statuts as $key => $label)
                                    <option value="{{ $key }}" @selected(old('statut', $evaluation->statut) == $key)>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="points_forts" rows="4" class="form-control rounded-4 @error('points_forts') is-invalid @enderror">{{ old('points_forts', $evaluation->points_forts) }}</textarea>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="points_a_ameliorer" rows="4" class="form-control rounded-4 @error('points_a_ameliorer') is-invalid @enderror">{{ old('points_a_ameliorer', $evaluation->points_a_ameliorer) }}</textarea>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="recommandations" rows="4" class="form-control rounded-4 @error('recommandations') is-invalid @enderror">{{ old('recommandations', $evaluation->recommandations) }}</textarea>
                        ...
                        <div class="col-12">
                            <div class="d-flex flex-wrap gap-2">
                                ...
                                <a href="{{ route('rh.evaluations.show', $evaluation) }}" class="btn btn-light rounded-pill px-4">Annuler</a>
                            </div>
                        </div>
                    </div>
                </form>
