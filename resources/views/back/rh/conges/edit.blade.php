...
@section('page_title', 'Modifier le congé')
@section('page_subtitle', 'Met à jour la demande de congé, ses dates, son statut et son circuit de validation.')
        ...
        <div class="col-12">
            <div class="content-card">
                <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
                    <div>
                        <h4 class="fw-bold mb-1">Modification de la demande</h4>
                        <p class="text-muted mb-0">
                            {{ optional($conge->membreEquipe)->nom }} {{ optional($conge->membreEquipe)->prenom }}
                        </p>
                    </div>
                    <div class="d-flex flex-wrap gap-2">
                        <a href="{{ route('rh.conges.show', $conge) }}" class="btn btn-outline-primary rounded-pill px-4">
                ...
                <form method="POST" action="{{ route('rh.conges.update', $conge) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <select name="membre_equipe_id" class="form-select custom-input @error('membre_equipe_id') is-invalid @enderror">
                                ...
                                @foreach($membres as $membre)
                                    <option value="{{ $membre->id }}" @selected(old('membre_equipe_id', $conge->membre_equipe_id) == $membre->id)>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="type_conge" class="form-select custom-input @error('type_conge') is-invalid @enderror">
                                @foreach($typesConge as $key => $label)
                                    <option value="{{ $key }}" @selected(old('type_conge', $conge->type_conge) == $key)>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" name="date_debut" class="form-control custom-input @error('date_debut') is-invalid @enderror"
                                   value="{{ old('date_debut', optional($conge->date_debut)->format('Y-m-d')) }}">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" name="date_fin" class="form-control custom-input @error('date_fin') is-invalid @enderror"
                                   value="{{ old('date_fin', optional($conge->date_fin)->format('Y-m-d')) }}">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" min="1" name="nombre_jours" class="form-control custom-input @error('nombre_jours') is-invalid @enderror"
                                   value="{{ old('nombre_jours', $conge->nombre_jours) }}">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="statut" class="form-select custom-input @error('statut') is-invalid @enderror">
                                @foreach($statuts as $key => $label)
                                    <option value="{{ $key }}" @selected(old('statut', $conge->statut) == $key)>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="valide_par" class="form-select custom-input @error('valide_par') is-invalid @enderror">
                                ...
                                @foreach($validateurs as $validateur)
                                    <option value="{{ $validateur->id }}" @selected(old('valide_par', $conge->valide_par) == $validateur->id)>
                                        {{ $validateur->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="motif" rows="5" class="form-control rounded-4 @error('motif') is-invalid @enderror">{{ old('motif', $conge->motif) }}</textarea>
                        ...
                        <div class="col-12">
                            <div class="d-flex flex-wrap gap-2">
                                ...
                                <a href="{{ route('rh.conges.show', $conge) }}" class="btn btn-light rounded-pill px-4">Annuler</a>
                            </div>
                        </div>
                    </div>
                </form>
