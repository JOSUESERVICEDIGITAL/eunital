@extends('back.layouts.principal')

@section('title', 'Présences RH')
@section('page_title', 'Présences RH')
@section('page_subtitle', 'Pilotage des présences, absences, retards et pointages avec filtres avancés et accès rapide aux vues temporelles.')

@section('content')
    @php
        $collection = $presences->getCollection();
        $totalPresents = $collection->where('statut', 'present')->count();
        $totalAbsents = $collection->where('statut', 'absent')->count();
        $totalRetards = $collection->where('statut', 'retard')->count();
    @endphp

    <div class="row g-4">

        <div class="col-12">
            <div class="row g-4">
                <div class="col-md-3">
                    <div class="content-card h-100">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <div class="mini-label">Total pointages</div>
                                <h3 class="stat-number">{{ $presences->total() }}</h3>
                            </div>
                            <div class="stat-icon bg-primary-subtle text-primary">
                                <i class="fa-solid fa-clock"></i>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="col-md-3">
                    <div class="content-card h-100">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <div class="mini-label">Présents</div>
                                <h3 class="stat-number">{{ $totalPresents }}</h3>
                            </div>
                            <div class="stat-icon bg-success-subtle text-success">
                                <i class="fa-solid fa-circle-check"></i>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="col-md-3">
                    <div class="content-card h-100">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <div class="mini-label">Absents</div>
                                <h3 class="stat-number">{{ $totalAbsents }}</h3>
                            </div>
                            <div class="stat-icon bg-danger-subtle text-danger">
                                <i class="fa-solid fa-user-xmark"></i>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="col-md-3">
                    <div class="content-card h-100">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <div class="mini-label">Retards</div>
                                <h3 class="stat-number">{{ $totalRetards }}</h3>
                            </div>
                            <div class="stat-icon bg-warning-subtle text-warning">
                                <i class="fa-solid fa-hourglass-half"></i>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        {{-- Bandeau actions --}}
        <div class="col-12">
            <div class="content-card">
                <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                    <div>
                        <h4 class="mb-1 fw-bold">Gestion des présences</h4>
                        <p class="text-muted mb-0">
                            Consulte les pointages, filtre les anomalies, ouvre les historiques par employé et navigue entre les vues temporelles.
                        </p>
                    </div>

                    <div class="d-flex flex-wrap gap-2">
                        <a href="{{ route('rh.presences.journalier') }}" class="btn btn-outline-success rounded-pill px-4">
                            <i class="fa-solid fa-calendar-day me-2"></i>Journalier
                        </a>
                        <a href="{{ route('rh.presences.retards') }}" class="btn btn-outline-warning rounded-pill px-4">
                            <i class="fa-solid fa-hourglass-half me-2"></i>Retards
                        </a>
                        <a href="{{ route('rh.presences.create') }}" class="btn btn-primary rounded-pill px-4">
                            <i class="fa-solid fa-plus me-2"></i>Nouvelle présence
                        </a>
                    </div>
                </div>
            </div>
        </div>

        {{-- Filtres --}}
        <div class="col-12">
            <div class="content-card">
                <form method="GET" action="{{ route('rh.presences.index') }}">
                    <div class="row g-3">
                        <div class="col-md-4">
                            <label class="form-label fw-semibold">Recherche</label>
                            <input type="text" name="search" class="form-control custom-input"
                                   value="{{ $filters['search'] ?? '' }}"
                                   placeholder="Nom, prénom, email, téléphone, observation...">
                        </div>

                        <div class="col-md-2">
                            <label class="form-label fw-semibold">Employé</label>
                            <select name="membre_equipe_id" class="form-select custom-input">
                                <option value="">Tous</option>
                                @foreach($membres as $membre)
                                    <option value="{{ $membre->id }}" @selected(($filters['membre_equipe_id'] ?? '') == $membre->id)>
                                        {{ $membre->nom }} {{ $membre->prenom }}
                                    </option>
                                @endforeach
                            </select>
                        </div>

                        <div class="col-md-2">
                            <label class="form-label fw-semibold">Département</label>
                            <select name="departement_id" class="form-select custom-input">
                                <option value="">Tous</option>
                                @foreach($departements as $departement)
                                    <option value="{{ $departement->id }}" @selected(($filters['departement_id'] ?? '') == $departement->id)>
                                        {{ $departement->nom }}
                                    </option>
                                @endforeach
                            </select>
                        </div>

                        <div class="col-md-2">
                            <label class="form-label fw-semibold">Statut</label>
                            <select name="statut" class="form-select custom-input">
                                <option value="">Tous</option>
                                @foreach($statuts as $key => $label)
                                    <option value="{{ $key }}" @selected(($filters['statut'] ?? '') == $key)>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        </div>

                        <div class="col-md-1">
                            <label class="form-label fw-semibold">Du</label>
                            <input type="date" name="date_presence_debut" class="form-control custom-input"
                                   value="{{ $filters['date_presence_debut'] ?? '' }}">
                        </div>

                        <div class="col-md-1">
                            <label class="form-label fw-semibold">Au</label>
                            <input type="date" name="date_presence_fin" class="form-control custom-input"
                                   value="{{ $filters['date_presence_fin'] ?? '' }}">
                        </div>
                    </div>

                    <div class="d-flex flex-wrap gap-2 mt-4">
                        <button type="submit" class="btn btn-primary rounded-pill px-4">
                            <i class="fa-solid fa-magnifying-glass me-2"></i>Filtrer
                        </button>
                        <a href="{{ route('rh.presences.index') }}" class="btn btn-outline-secondary rounded-pill px-4">
                            <i class="fa-solid fa-rotate-left me-2"></i>Réinitialiser
                        </a>
                    </div>
                </form>
            </div>
        </div>

        {{-- Tableau --}}
        <div class="col-12">
            <div class="content-card">
                <div class="table-head-custom mb-4">
                    <div>
                        <h5 class="mb-1 fw-bold">Liste des présences</h5>
                        <p class="text-muted mb-0">Lecture rapide des pointages enregistrés dans le système.</p>
                    </div>
                </div>

                <div class="table-responsive">
                    <table class="table align-middle custom-table">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Employé</th>
                                <th>Département</th>
                                <th>Date</th>
                                <th>Arrivée</th>
                                <th>Départ</th>
                                <th>Statut</th>
                                <th class="text-end">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($presences as $presence)
                                <tr>
                                    <td>{{ $presence->id }}</td>
                                    <td>
                                        <div class="d-flex align-items-center gap-3">
                                            <div class="table-avatar bg-success-subtle text-success">
                                                <i class="fa-solid fa-user-clock"></i>
                                            </div>
                                            <div>
                                                <div class="fw-bold">
                                                    {{ optional($presence->membreEquipe)->nom }} {{ optional($presence->membreEquipe)->prenom }}
                                                </div>
                                                <div class="text-muted small">
                                                    {{ optional(optional($presence->membreEquipe)->poste)->nom ?? 'Poste non défini' }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>{{ optional(optional($presence->membreEquipe)->departement)->nom ?? '—' }}</td>
                                    <td>{{ $presence->date_presence?->format('d/m/Y') ?? '—' }}</td>
                                    <td>{{ $presence->heure_arrivee ?? '—' }}</td>
                                    <td>{{ $presence->heure_depart ?? '—' }}</td>
                                    <td>
                                        @php
                                            $statusClass = match($presence->statut) {
                                                'present' => 'text-bg-success',
                                                'absent' => 'text-bg-danger',
                                                'retard' => 'text-bg-warning',
                                                'mission' => 'text-bg-info',
                                                'teletravail' => 'text-bg-primary',
                                                'conge' => 'text-bg-secondary',
                                                default => 'text-bg-light'
                                            };
                                        @endphp
                                        <span class="badge rounded-pill {{ $statusClass }}">
                                            {{ ucfirst(str_replace('_', ' ', $presence->statut)) }}
                                        </span>
                                    </td>
                                    <td class="text-end">
                                        <div class="d-inline-flex flex-wrap gap-2 justify-content-end">
                                            <a href="{{ route('rh.presences.show', $presence) }}" class="btn btn-sm btn-light rounded-pill px-3">
                                                <i class="fa-solid fa-eye me-1"></i>Voir
                                            </a>

                                            <a href="{{ route('rh.presences.edit', $presence) }}" class="btn btn-sm btn-warning rounded-pill px-3">
                                                <i class="fa-solid fa-pen me-1"></i>Modifier
                                            </a>

                                            @if($presence->membreEquipe)
                                                <a href="{{ route('rh.presences.par-employe', $presence->membreEquipe) }}" class="btn btn-sm btn-outline-primary rounded-pill px-3">
                                                    <i class="fa-solid fa-user me-1"></i>Employé
                                                </a>
                                            @endif

                                            <button type="button"
                                                    class="btn btn-sm btn-outline-danger rounded-pill px-3"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#deletePresenceModal{{ $presence->id }}">
                                                <i class="fa-solid fa-trash me-1"></i>Supprimer
                                            </button>
                                        </div>

                                        <div class="modal fade" id="deletePresenceModal{{ $presence->id }}" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content border-0 rounded-4">
                                                    <div class="modal-header border-0">
                                                        <h5 class="modal-title fw-bold">Supprimer la présence</h5>
                                                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                    </div>
                                                    <div class="modal-body">
                                                        Veux-tu vraiment supprimer cet enregistrement de présence ?
                                                    </div>
                                                    <div class="modal-footer border-0">
                                                        <button type="button" class="btn btn-light rounded-pill px-4" data-bs-dismiss="modal">Annuler</button>
                                                        <form method="POST" action="{{ route('rh.presences.destroy', $presence) }}">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="submit" class="btn btn-danger rounded-pill px-4">Supprimer</button>
                                                        </form>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center py-5">
                                        <div class="empty-state">
                                            <i class="fa-solid fa-clock empty-state-icon"></i>
                                            <h5 class="mt-3">Aucune présence trouvée</h5>
                                            <p class="text-muted">Crée un pointage ou ajuste les filtres.</p>
                                            <a href="{{ route('rh.presences.create') }}" class="btn btn-primary rounded-pill px-4">
                                                <i class="fa-solid fa-plus me-2"></i>Nouvelle présence
                                            </a>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>

                <div class="mt-4">
                    {{ $presences->links() }}
                </div>
            </div>
        </div>

    </div>

    <style>
        .mini-label{font-size:13px;color:#64748b;font-weight:700;margin-bottom:8px}
        .stat-number{font-size:32px;font-weight:800;margin:0}
        .stat-icon{width:58px;height:58px;border-radius:18px;display:flex;align-items:center;justify-content:center;font-size:22px}
        .custom-input{height:48px;border-radius:16px}
        .table-head-custom{display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap}
        .custom-table thead th{font-size:13px;text-transform:uppercase;letter-spacing:.5px;color:#64748b;border-bottom:1px solid #e5e7eb}
        .custom-table tbody td{border-bottom:1px solid #f1f5f9}
        .table-avatar{width:48px;height:48px;border-radius:16px;display:flex;align-items:center;justify-content:center;font-size:18px}
        .empty-state{padding:20px}
        .empty-state-icon{font-size:42px;color:#94a3b8}
    </style>
@endsection